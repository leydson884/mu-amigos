...
class CTM_PanelAdmin extends CTM_MSSQL
{
	...
	{
		global $CTM_General, $CTM_Crypt, $CTM_Template;
		...
		$CTM_Template->Set("EffectWeb_Version", base64_decode(constant("Web_Version")));
	...
	private function Add_CronbTab()
	{
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			if(empty($Hour)) { $Error[0] .= "&raquo; Hora para Efetuar o CronTab<br />\n"; }
			if(empty($Hour)) { $Error[0] .= "&raquo; Minuto para Efetuar o CronTab<br />\n"; }
	...
	private function List_CronTab()
	{
		...
		if($Check < 1)
	   	...
	   	{
		  	$Query = $this->Query("SELECT Id,CronTab,Cron_Date,Cron_Time FROM dbo.{$CTM[13]} ORDER BY Id DESC");
		   	while($Get = $this->Fetch($Query))
		   	{
				switch($Get[1])
				{
			   	...
			   	$Return .= "<option value=\"{$Get[0]}\">{$Name} - {$Get[3]} (".date("d/M/Y", $Get[2]).")</option>\n";
		   	}
	...
	private function Manage_CronTab()
	{
		...
		$CTM_Template->Set("CronTab_Hour", $Cron_Time[0]);
		$CTM_Template->Set("CronTab_Minute", $Cron_Time[1]);
	...
	private function Manage_Poll()
	{
		...
		if(empty($_POST["Id"]))
		{
			exit("<div class=\"warning-box\"> Slecione a Enquete.</div>");
		}
		elseif($Check < 1)
		{
			exit("<div class=\"error-box\"> Esta enquete n&atilde;o existe!</div>");
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			else
			{
				...
				if($_PanelAdmin["Accounts"]["Ban"]["Send_Mail"] === TRUE)
				{
					...
					{
						$this->Query("INSERT INTO dbo.{$CTM[7]} (Account,Reason,Time) VALUES ('{$Account}','{$Reason}',{$Str_Time})");
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			else
			{
				...
				if($_PanelAdmin["Characters"]["Ban"]["Send_Mail"] === TRUE)
				{
					...
					{
						$this->Query("INSERT INTO dbo.{$CTM[8]} (Character,Time,Reason) VALUES ('{$Character}',".strtotime("+ {$Time} days").",'{$Reason}')");
	...
	private function Add_VIP()
	{
		if($_GET["cmd"] == TRUE)
		{
			...
			$Check[0] = $this->NumQuery("SELECT * FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE memb___id='{$Account}'");
			$Check[1] = $this->FetchQuery("SELECT ".VIP_Column.",".VIP_Time." FROM ".VIP_DB.".dbo.".VIP_Table." WHERE ".VIP_Login."='{$Account}'");
			...
			if($Define == FALSE)
			{
				if($Check[1][0] == $Type)
				{
					$VIP_Time = strtotime(date("d-m-Y g:i a", $Check[1][1])." + ".$Time." days");
				}
				else
				{
				...
				$this->Query("UPDATE ".VIP_DB.".dbo.".VIP_Table." SET ".VIP_Column."={$Type},".VIP_Time."={$VIP_Time},".VIP_Credits."=".VIP_Credits."+{$Time}{$Begin} WHERE ".VIP_Login."='{$Account}'");
				exit("<div class=\"success-box\"> Adicionado <b>".$Time." dias</b> de <b>".constant("VIP_".$Type)."</b> na conta <b>".$Account."</b> com Sucesso!</div>");
			}
		}
	}
	private function Transform_VIP()
	{
		if($_GET["cmd"] == TRUE)
		{
			...
			{
				$this->Query("UPDATE ".VIP_DB.".dbo.".VIP_Table." SET ".VIP_Column."={$Type} WHERE ".VIP_Login."='{$Account}'");
	...
	private function Delete_VIP()
	{
		if($_GET["cmd"] == TRUE)
		{
			$Account = $_POST["Account"];
			$Check[0] = $this->NumQuery("SELECT * FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE memb___id='{$Account}'");
			$Check[1] = $this->FetchQuery("SELECT ".VIP_Column." FROM ".VIP_DB.".dbo.".VIP_Table." WHERE ".VIP_Login."='{$Account}'");
			...
			{
				$this->Query("UPDATE ".VIP_DB.".dbo.".VIP_Table." SET ".VIP_Column."=0,".VIP_Begin."=0,".VIP_Time."=0,".VIP_Credits."=0 WHERE ".VIP_Login."='{$Account}'");
	...
	private function Add_Cash()
	{
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			{
				$CTM_General->Check_Coin_Table($Account);
				$this->Query("UPDATE ".GL_DB.".dbo.".GL_Table." SET ".constant("GL_Column_".$Coin)."=".constant("GL_Column_".$Coin)."+{$Cash} WHERE ".GL_Login."='{$Account}'");
	...
	private function Delete_Cash()
	{
		if($_GET["cmd"] == TRUE)
		{
			...
			$Coin = $_POST["Coin"];
			$Check[0] = $this->NumQuery("SELECT * FROM ".MuAcc_DB.".dbo.MEMB_INFO WHERE memb___id='{$Account}'");
			$Check[1] = $this->FetchQuery("SELECT ".constant("GL_Column_".$Coin)." FROM ".GL_DB.".dbo.".GL_Table." WHERE ".GL_Login."='{$Account}'");

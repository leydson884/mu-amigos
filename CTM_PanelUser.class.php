...
class CTM_PanelUser extends CTM_MSSQL
{
	...
	{
		global $CTM_General, $CTM_Template, $_Panel, $_PmSystem;
	...
	private function General_Nav()
	{
		...
		$CTM_Template->Set("Character_Manage", isset($_SESSION["Web_ManageChar"]) == TRUE ? "<script>CTM_Load('?pag=paneluser&str=CHAR_DIV','Char_Div','GET');</script>" : NULL);
	...
	private function Change_PID()
	{
		if($_GET["cmd"] == TRUE)
		{
			...
			{
				exit("<div class=\"warning-box\"> O Perosnal ID deve conterm apenas numeros.</div>");
			}
	...
	{		
		...
		if($_GET["cmd"] == TRUE)
		{
			$Vault_Size = constant("GS_Version") >= 2 ? 1920 : 1200;
			$Item_Size = constant("GS_Version") >= 2 ? 32 : 20;
			
			for($WzAG = 0; $WzAG < $Vault_Size / 216; ++$WzAG)
			{
				$Code = $WzAG * 255;
				$Tech = $Code + 255;
			...
			for($GsAG = 0; $GsAG < strlen($Vault) * 2; $GsAG++)
			{
				if (($GsAG % $Item_Size == 0))
				{
					$Binarry = substr($Item_Hex, $GsAG, $Item_Size);
	...
	private function ResetSystem()
	{
		...
		{
			case 0 :
			$Reset["Level"] = $_Panel["Char"]["Reset"]["General"]["Level"][0];
			$Reset["Money"] = $_Panel["Char"]["Reset"]["General"]["Money"][0];
			$Reset["Return"] = $_Panel["Char"]["Reset"]["General"]["Return"][0];
			$Reset["Clear_Inv"] = $_Panel["Char"]["Reset"]["General"]["Invent"][0] == 1 ? "Inventory=NULL," : NULL;
			$Reset["Clear_Skill"] = $_Panel["Char"]["Reset"]["General"]["Skill"][0] == 1 ? "MagicList=NULL," : NULL;
			$Reset["Clear_Quest"] = $_Panel["Char"]["Reset"]["General"]["Quest"][0] == 1 ? "Quest=NULL,Class=".$this->ResetClass($Find_Char["Class"])."," : NULL;
			$Reset["Inventory"] = $_Panel["Char"]["Reset"]["General"]["Invent"][0] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Skills"] = $_Panel["Char"]["Reset"]["General"]["Skill"][0] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Quests"] = $_Panel["Char"]["Reset"]["General"]["Quest"][0] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Points"] = $_Find["Point"][0];
			$Reset["Class"] = $_Panel["Char"]["Reset"]["General"]["Quest"][0] == 1 ? $this->ResetClass($Find_Char["Class"]) : $Find_Char["Class"];
			break;
			case 1 :
			$Reset["Level"] = $_Panel["Char"]["Reset"]["General"]["Level"][1];
			$Reset["Money"] = $_Panel["Char"]["Reset"]["General"]["Money"][1];
			$Reset["Return"] = $_Panel["Char"]["Reset"]["General"]["Return"][1];
			$Reset["Clear_Inv"] = $_Panel["Char"]["Reset"]["General"]["Invent"][1] == 1 ? "Inventory=NULL," : NULL;
			$Reset["Clear_Skill"] = $_Panel["Char"]["Reset"]["General"]["Skill"][1] == 1 ? "MagicList=NULL," : NULL;
			$Reset["Clear_Quest"] = $_Panel["Char"]["Reset"]["General"]["Quest"][1] == 1 ? "Quest=NULL,Class=".$this->ResetClass($Find_Char["Class"])."," : NULL;
			$Reset["Inventory"] = $_Panel["Char"]["Reset"]["General"]["Invent"][1] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Skills"] = $_Panel["Char"]["Reset"]["General"]["Skill"][1] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Quests"] = $_Panel["Char"]["Reset"]["General"]["Quest"][1] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Points"] = $_Find["Point"][1];
			$Reset["Class"] = $_Panel["Char"]["Reset"]["General"]["Quest"][1] == 1 ? $this->ResetClass($Find_Char["Class"]) : $Find_Char["Class"];
			break;
			case 2:
			$Reset["Level"] = $_Panel["Char"]["Reset"]["General"]["Level"][2];
			$Reset["Money"] = $_Panel["Char"]["Reset"]["General"]["Money"][2];
			$Reset["Return"] = $_Panel["Char"]["Reset"]["General"]["Return"][2];
			$Reset["Clear_Inv"] = $_Panel["Char"]["Reset"]["General"]["Invent"][2] == 1 ? "Inventory=NULL," : NULL;
			$Reset["Clear_Skill"] = $_Panel["Char"]["Reset"]["General"]["Invent"][2] == 1 ? "MagicList=NULL," : NULL;
			$Reset["Clear_Quest"] = $_Panel["Char"]["Reset"]["General"]["Quest"][2] == 1 ? "Quest=NULL,Class=".$this->ResetClass($Find_Char["Class"])."," : NULL;
			$Reset["Inventory"] = $_Panel["Char"]["Reset"]["General"]["Invent"][2] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Skills"] = $_Panel["Char"]["Reset"]["General"]["Skill"][2] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Quests"] = $_Panel["Char"]["Reset"]["General"]["Quest"][2] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Points"] = $_Find["Point"][2];
			$Reset["Class"] = $_Panel["Char"]["Reset"]["General"]["Quest"][2] == 1 ? $this->ResetClass($Find_Char["Class"]) : $Find_Char["Class"];
			break;
			case 3:
			$Reset["Level"] = $_Panel["Char"]["Reset"]["General"]["Level"][3];
			$Reset["Money"] = $_Panel["Char"]["Reset"]["General"]["Money"][3];
			$Reset["Return"] = $_Panel["Char"]["Reset"]["General"]["Return"][3];
			$Reset["Clear_Inv"] = $_Panel["Char"]["Reset"]["General"]["Invent"][3] == 1 ? "Inventory=NULL," : NULL;
			$Reset["Clear_Skill"] = $_Panel["Char"]["Reset"]["General"]["Invent"][3] == 1 ? "MagicList=NULL," : NULL;
			$Reset["Clear_Quest"] = $_Panel["Char"]["Reset"]["General"]["Quest"][3] == 1 ? "Quest=NULL,Class=".$this->ResetClass($Find_Char["Class"])."," : NULL;
			$Reset["Inventory"] = $_Panel["Char"]["Reset"]["General"]["Invent"][3] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Skills"] = $_Panel["Char"]["Reset"]["General"]["Skill"][3] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Quests"] = $_Panel["Char"]["Reset"]["General"]["Quest"][3] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Points"] = $_Find["Point"][3];
			$Reset["Class"] = $_Panel["Char"]["Reset"]["General"]["Quest"][3] == 1 ? $this->ResetClass($Find_Char["Class"]) : $Find_Char["Class"];
			break;
			case 4:
			$Reset["Level"] = $_Panel["Char"]["Reset"]["General"]["Level"][4];
			$Reset["Money"] = $_Panel["Char"]["Reset"]["General"]["Money"][4];
			$Reset["Return"] = $_Panel["Char"]["Reset"]["General"]["Return"][4];
			$Reset["Clear_Inv"] = $_Panel["Char"]["Reset"]["General"]["Invent"][4] == 1 ? "Inventory=NULL," : NULL;
			$Reset["Clear_Skill"] = $_Panel["Char"]["Reset"]["General"]["Invent"][4] == 1 ? "MagicList=NULL," : NULL;
			$Reset["Clear_Quest"] = $_Panel["Char"]["Reset"]["General"]["Quest"][4] == 1 ? "Quest=NULL,Class=".$this->ResetClass($Find_Char["Class"])."," : NULL;
			$Reset["Inventory"] = $_Panel["Char"]["Reset"]["General"]["Invent"][4] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Skills"] = $_Panel["Char"]["Reset"]["General"]["Skill"][4] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Quests"] = $_Panel["Char"]["Reset"]["General"]["Quest"][4] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Points"] = $_Find["Point"][4];
			$Reset["Class"] = $_Panel["Char"]["Reset"]["General"]["Quest"][4] == 1 ? $this->ResetClass($Find_Char["Class"]) : $Find_Char["Class"];
			break;
			case 5:
			$Reset["Level"] = $_Panel["Char"]["Reset"]["General"]["Level"][5];
			$Reset["Money"] = $_Panel["Char"]["Reset"]["General"]["Money"][5];
			$Reset["Return"] = $_Panel["Char"]["Reset"]["General"]["Return"][5];
			$Reset["Clear_Inv"] = $_Panel["Char"]["Reset"]["General"]["Invent"][5] == 1 ? "Inventory=NULL," : NULL;
			$Reset["Clear_Skill"] = $_Panel["Char"]["Reset"]["General"]["Invent"][5] == 1 ? "MagicList=NULL," : NULL;
			$Reset["Clear_Quest"] = $_Panel["Char"]["Reset"]["General"]["Quest"][5] == 1 ? "Quest=NULL,Class=".$this->ResetClass($Find_Char["Class"])."," : NULL;
			$Reset["Inventory"] = $_Panel["Char"]["Reset"]["General"]["Invent"][5] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Skills"] = $_Panel["Char"]["Reset"]["General"]["Skill"][5] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Quests"] = $_Panel["Char"]["Reset"]["General"]["Quest"][5] == 1 ? "<span style=\"color: red;\">Sim</span>" : "<span style=\"color: green;\">N&atilde;o</span>";
			$Reset["Points"] = $_Find["Point"][5];
			$Reset["Class"] = $_Panel["Char"]["Reset"]["General"]["Quest"][5] == 1 ? $this->ResetClass($Find_Char["Class"]) : $Find_Char["Class"];
			break;
		}
		...
		if($Find_Char[Column_Reset] == 0)
		{
			$_Increment = $Find_Char[Column_Reset] * $Reset["Points"] + $Reset["Points"];
		}
		else
		{
			$_Increment = $Find_Char[Column_Reset] * $Reset["Points"];
		}
		switch($_Panel["Char"]["Reset"]["General"]["Mode"])
		{
			case 1 : 
			$Reset["New_Pts"] = $Find_Char["LevelUpPoint"] + $_Increment;
			break;
			case 2 :
			$Reset["New_Pts"] = $Reset["Points"] * $Find_Char[Column_Reset];
			break;
		}
	...
	private function Change_Class()
	{
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			{
				$this->Query("UPDATE ".MuGen_DB.".dbo.Character SET Class={$Class},MagicList=NULL WHERE Name='{$Character}' and AccountID='{$this->Login}'");
	...
	private function Profile_Char()
	{
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			{
				case 0 : $Message = "Abilitado"; break;
	...
	private function Reset_Points()
	{
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			else
			{
				...
				{
					$this->Query("UPDATE ".MuGen_DB.".dbo.Character SET Strength=".$this->ResetPoints(0).",Dexterity=".$this->ResetPoints(1).",Vitality=".$this->ResetPoints(2).",Energy=".$this->ResetPoints(3).",LevelUpPoint=LevelUpPoint+{$Total} WHERE Name='{$Character}' and AccountID='{$this->Login}'");
				}
		...
		if($_GET["cmd"] == TRUE)
		{
			...
			else
			{
				if($Inventory == TRUE)
				{
					...
					$this->Query("UPDATE ".MuGen_DB.".dbo.Character SET Inventory=NULL WHERE Name='{$Character}' and AccountID='{$this->Login}'");
				}
				if($Skills == TRUE)
				{
					$this->Selected .= "&raquo; Skills Limpas<br />\n";
					$this->Query("UPDATE ".MuGen_DB.".dbo.Character SET MagicList=NULL WHERE Name='{$Character}' and AccountID='{$this->Login}'");
				}
				if($Money == TRUE)
				{
				...
				if($Quests == TRUE)
				{
					...
					$this->Query("UPDATE ".MuGen_DB.".dbo.Character SET Quest=NULL,Class={$Reset_Class} WHERE Name='{$Character}' and AccountID='{$this->Login}'");
				}
	...
	{		
		if($_GET["cmd"] == TRUE)
		{
			...
			$Inventory_Size = constant("GS_Version") >= 2 ? 1728 : 760;
			$Item_Size = constant("GS_Version") >= 2 ? 32 : 20;
			
			for($WzAG = 0; $WzAG < $Inventory_Size / 216; ++$WzAG)
			{
				$Code = $WzAG * 255;
				$Tech = $Code + 255;
			...
			for($GsAG = 0; $GsAG < strlen($Inventory) * 2; $GsAG++)
			{
				if (($GsAG % $Item_Size == 0))
				{
					$Binarry = substr($Item_Hex, $GsAG, $Item_Size);
